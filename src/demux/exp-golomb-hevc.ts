...
class PTL implements HevcPTL {
  profile_space = 0;
  tier_flag = 0;
  profile_idc = 0;
  profile_compatibility_flags = 0xffffffff;
  constraint_indicator_flags_high_16 = 0xffffffff;
  constraint_indicator_flags_low_32= 0xffffffff;
  level_idc = 0;
}

class VUI implements HevcVUI {
  sar_width = 1;
  sar_height = 1;
...
class ExpGolombHEVC extends ExpGolomb {
   ...
   */
  static removeEmulation(data: Uint8Array): Uint8Array {
 ...
 /**
   ...
   */
  private readPtl(max_sub_layers_minus1: number): HevcPTL {
    const readBits = this.readBits.bind(this);
    const ptl = new PTL();
   ...
   */
  private read_short_term_ref_set(num_short_term_ref_pic_sets: number)
  {
    const readBits = this.readBits.bind(this);
    const readUEG = this.readUEG.bind(this);
   ...
   * @returns
   */
  private read_vui(max_sub_layers_minus1: number): HevcVUI
  {
    const readBits = this.readBits.bind(this);
    const readUEG = this.readUEG.bind(this);
  ...
  private read_vui_headers(max_sub_layers_minus1: number, vui: HevcVUI)
  {
    const readBits = this.readBits.bind(this);
    const readUEG = this.readUEG.bind(this);
  ...
  private read_vui_sub_layer_header(cpb_cnt_minus1: number, vui: HevcVUI)
  {
    const readBits = this.readBits.bind(this);
    const readUEG = this.readUEG.bind(this);
